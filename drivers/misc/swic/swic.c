...
static struct pi3a6386_swic *swic_info = NULL;
...
static swic_debug_accessory_enum swic_debug_accessory = SWIC_NO_DEBUG_ACCESSORY;
...
EXPORT_SYMBOL_GPL(swic_detect_audio);

void swic_notify_from_cclogic(swic_state_enum en_state, swic_debug_accessory_enum dbg_acc)
{
	SWIC_INF_PRINT("%s:### en_state=[0x%02x] dbg_state=[0x%02x]\n", __func__, en_state, dbg_acc);

	if (swic_info == NULL) {
		return;
	}

	swic_debug_accessory = dbg_acc;
...
EXPORT_SYMBOL_GPL(swic_notify_from_cclogic);

bool swic_is_debug_accessory(void) {
	if (swic_debug_accessory == SWIC_DEBUG_ACCESSORY) {
		return true;
	} else {
		return false;
	}
}
EXPORT_SYMBOL_GPL(swic_is_debug_accessory);
...
static int32_t swic_earphone_in_proc_reg_w_all_sw_on(void)
{
	...
	else
	{
		...
		SWIC_DBG_PRINT("%s:###[GPIO] HS_DET_EN =High\n", __func__);
...
static int32_t swic_insert_earphone_proc(void)
{
	...
	SWIC_INF_PRINT("ccic_swic_rcv:%02X,%02X,%02X,%02X,%02X,%02X",dm_info_ccic_reg01_state, dm_info_ccic_reg02_state, dm_info_ccic_reg03_state, dm_info_ccic_reg04_state, dm_info_swic_reg00_state, dm_info_swic_reg01_state);
...
static int32_t swic_remove_earphone_proc(void)
{
	...
	SWIC_DBG_PRINT("%s:### [GPIO] HS_DET_EN =Low\n", __func__);
...
EXPORT_SYMBOL_GPL(swic_get_dminfo);
#endif
static void swic_fix_accessory(u8 val)
{
	int32_t i;
...
EXPORT_SYMBOL_GPL(swic_set_dminfo_from_cclogic);
...
EXPORT_SYMBOL_GPL(swic_reg_cbfunc);
...
EXPORT_SYMBOL_GPL(swic_unreg_cbfunc);

swic_accessory_enum swic_get_accessory(void)
{
	return swic_set_mode;
}
EXPORT_SYMBOL_GPL(swic_get_accessory);
