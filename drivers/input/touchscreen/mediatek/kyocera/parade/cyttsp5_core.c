...
#define KC_TOUCH_RESTART_COUNT_MAX		5
...
static void cyttsp5_watchdog_work(struct work_struct *work)
{
	...
	if(cd->irq_enabled == false){
		pr_info("%s watch dog skip because irq is disable\n",__func__);
		goto queue_startup;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
...
#else
		if(!atomic_read(&cd->loader_finished)){
			rc = wait_for_completion_interruptible_timeout(&cd->fw_update_done, 60 * HZ);
			if (rc <= 0) {
				pr_err("%s: Error FWDL timeout. rc = %d\n", __func__, rc);
			}
		}
	...
	case KC_TOUCH_POWER_OFF:
		...
		if (cd->cpdata->power) {
			rc = cd->cpdata->power(cd->cpdata, 0, dev, 0);
			if (rc < 0) {
				dev_err(dev, "%s: HW Power up fails r=%d\n", __func__, rc);
			}
		}
...
static ssize_t cyttsp5_mode_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
static ssize_t cyttsp5_factorylock_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
extern u32 pt_kc_get_fw_ver_image(void);
extern u32 pt_kc_get_fw_revctrl_image(void);
extern u16 pt_kc_get_fw_config_ver_image(void);

static ssize_t fw_ver_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int status = -1;
	int rc = 0;
	...
	u32 fw_ver_image = 0;
	u32 fw_revctrl_image = 0;
	u16 fw_config_ver_image = 0;

	u32 fw_ver_ic = 0;
	u32 fw_revctrl_ver_ic = 0;
	u16 config_ver_ic = 0;

	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_cydata *cydata = &cd->sysinfo.cydata;

	fw_ver_ic = cydata->fw_ver_major << 8;
	fw_ver_ic += cydata->fw_ver_minor;
	fw_revctrl_ver_ic = cydata->revctrl;
	config_ver_ic = cydata->fw_ver_conf;


	fw_ver_image = pt_kc_get_fw_ver_image();
	fw_revctrl_image = pt_kc_get_fw_revctrl_image();
	fw_config_ver_image = pt_kc_get_fw_config_ver_image();


	if(fw_ver_image != fw_ver_ic) {
		pr_err("%s: FW version is not same!\n", __func__);
		rc |= 1;
	}
	if(fw_revctrl_image != fw_revctrl_ver_ic) {
		pr_err("%s: FW revctrl version is not same!\n", __func__);
		rc |= 2;
	}
	if(fw_config_ver_image != config_ver_ic) {
		pr_err("%s: FW Config version is not same!\n", __func__);
		rc |= 4;
	}
	if(!rc){
		status = 0;
	...
	return sprintf(buf,
			"Result: %s\n"
			"Status: %d\n"
			"FW ic_ver: 0x%04X.0x%08X.0x%04X\n"
			"FW img_ver: 0x%04X.0x%08X.0x%04X\n", 
				(status==0) ? KTP_RET_OK : KTP_RET_NG, 
				rc,
				fw_ver_ic, fw_revctrl_ver_ic, config_ver_ic,
				fw_ver_image, fw_revctrl_image, fw_config_ver_image);
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(mode_check, S_IRUGO, cyttsp5_mode_check_show, NULL),
	__ATTR(factory_status, S_IRUSR|S_IRGRP, cyttsp5_factorylock_show, NULL),
	__ATTR(fw_ver_check, S_IRUGO, fw_ver_check_show, NULL),
};
...
static void cyttsp5_startup_sync(struct cyttsp5_core_data *cd)
{
	int rc = 0;
	int startup_flg = 0;
	...
	if (cd->startup_state == STARTUP_NONE) {
		cd->startup_state = STARTUP_QUEUED;
		dev_err(cd->dev, "%s: cyttsp5_startup\n", __func__);
		startup_flg = 1;
	} else {
		dev_dbg(cd->dev, "%s: startup_state = %d\n", __func__,
			cd->startup_state);
...
static void cyttsp5_poweroffon(struct work_struct *work)
{
	struct cyttsp5_core_data *cd = container_of(work, struct cyttsp5_core_data, restart_work);
	...
	if(cd->factory_state){
		pr_info("%s: Factory Locked!\n", __func__);
		goto exit;
	}

	cyttsp5_startup_sync(cd);
		...
		unsigned long event, void *data)
{
	...
	if ((event != FB_EVENT_BLANK && event != FB_EARLY_EVENT_BLANK) || !evdata)
		goto exit_out;
	...
	if(!atomic_read(&cd->loader_finished)){
		pr_info("%s: cyttsp5 FW loading -> exit\n", __func__);
		goto exit;
	}

	if (flush_work(&cd->resume_work))
		pr_warn("%s: waited resume worker finished\n", __func__);

	blank = evdata->data;
	switch (event) {
		case FB_EARLY_EVENT_BLANK:
			pr_info("%s: [%ld]FB_EARLY_EVENT_BLANK\n", __func__, event);

			if (*blank == FB_BLANK_UNBLANK) {
				dev_info(cd->dev, "%s: EARLY UNBLANK!\n", __func__);
			} else if (*blank == FB_BLANK_POWERDOWN) {
				dev_info(cd->dev, "%s: EARLY POWERDOWN!\n", __func__);

				dev_info(cd->dev, "%s: restart count=%d\n", __func__, cd->restart_count);

				if ((atomic_read(&cd->oem_state)&OEM_STATE_RESUME_BIT)) {
					cd->restart_count++;
					if (cd->restart_count >= KC_TOUCH_RESTART_COUNT_MAX) {
						dev_notice(cd->dev, "%s: start restart_work count=%d\n", __func__, cd->restart_count);
						schedule_work(&cd->restart_work);
						cd->restart_count = 0;
					}
				}
			}
...
//	cd->irq = gpio_to_irq(cd->cpdata->irq_gpio);
	pr_err("%s gpio_to_irq(gpio %d) = %d\n",__func__, cd->cpdata->irq_gpio, cd->irq);
	if (cd->irq < 0)
		return -EINVAL;
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	gpio_set_value(cd->cpdata->avdd_gpio, 1);
	usleep_range(1000,1000);
	gpio_set_value(cd->cpdata->vdd_gpio, 1);
	usleep_range(1000,1000);
	...
	pr_err("%s TS_INT pin select\n", __func__);

	pr_err("%s I2C_TP pin select\n", __func__);
...
#endif /* CONFIG_TOUCHSCREEN_KC_TOUCHMODE_ENABLED */
	INIT_WORK(&cd->restart_work, cyttsp5_poweroffon);
...
int cyttsp5_shutdown(struct cyttsp5_core_data *cd)
{
	...
	usleep_range(1000,1000);
#endif
	gpio_set_value(cd->cpdata->vdd_gpio, 0);
	gpio_set_value(cd->cpdata->avdd_gpio, 0);
