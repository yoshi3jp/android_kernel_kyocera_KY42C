...
static struct wakeup_source touchpanel_wakeup_source;
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	__pm_wakeup_event(&touchpanel_wakeup_source, 2 * HZ); 
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
...
/*	cyttsp5_mt_send_dummy_event(md); */
	input_report_key(md->input, BTN_EASYWAKE_UP, 1);
	input_sync(md->input);
	input_report_key(md->input, BTN_EASYWAKE_UP, 0);
	input_sync(md->input);
	mutex_unlock(&md->mt_lock);
	return 0;
}
...
#endif
	__set_bit(BTN_EASYWAKE_UP, md->input->keybit);
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	wakeup_source_init(&touchpanel_wakeup_source, dev_name(dev));
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
