		...
		struct device_node *core_node)
{
	...
	/* Required fields */
	rc = of_property_read_u32(core_node, "cy,irq_gpio_num", &value);
	...
	value = of_get_named_gpio(core_node, "cy,vdd_18_gpio", 0);
	pr_info("%s: cyttsp5: vdd_18_gpio value = %d\n",__func__,value);
	if (rc)
		goto fail_free;
	if (gpio_is_valid(value))
		pdata->vdd_gpio = value;

	value = of_get_named_gpio(core_node, "cy,avdd_30_gpio", 0);
	pr_info("%s: cyttsp5: avdd_30_gpio value = %d\n",__func__,value);
	if (rc)
		goto fail_free;
	if (gpio_is_valid(value))
