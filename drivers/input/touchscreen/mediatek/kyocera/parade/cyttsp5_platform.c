		...
		struct device *dev)
{
	...
	gpio_set_value(pdata->avdd_gpio, 0);

	msleep(20);

	gpio_set_value(pdata->avdd_gpio, 1);
...
{
	pr_err("[KCTOUCH]%s Power ON start\n", __func__);
	gpio_set_value(pdata->avdd_gpio, 1);
	usleep_range(1000,1000);
	gpio_set_value(pdata->vdd_gpio, 1);
	usleep_range(1000,1000);
...
static int cyttsp5_sleep(struct cyttsp5_core_platform_data *pdata,
		struct device *dev, atomic_t *ignore_irq)
{
	pr_err("[KCTOUCH]%s Power OFF start\n", __func__);
	gpio_set_value(pdata->rst_gpio, 0);
	usleep_range(4000,4000);
	...
	gpio_set_value(pdata->avdd_gpio, 0);
	usleep_range(1000,1000);
	pr_err("[KCTOUCH]%s Power OFF end\n", __func__);
