...
u32 ktp_fw_ver_image = 0;
u32 ktp_fw_revctrl_image = 0;
u16 ktp_fw_config_ver_image = 0;

u32 pt_kc_get_fw_ver_image(void)
{
	return ktp_fw_ver_image;
}
EXPORT_SYMBOL(pt_kc_get_fw_ver_image);

u32 pt_kc_get_fw_revctrl_image(void)
{
	return ktp_fw_revctrl_image;
}
EXPORT_SYMBOL(pt_kc_get_fw_revctrl_image);

u16 pt_kc_get_fw_config_ver_image(void)
{
	return ktp_fw_config_ver_image;
}
EXPORT_SYMBOL(pt_kc_get_fw_config_ver_image);
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	dev_notice(dev, "%s: img vers:0x%04X new vers:0x%04X\n", __func__,
	...
	if (fw_ver_new < fw_ver_img) {
		dev_notice(dev, "%s: Image is older, will NOT upgrade\n",
				__func__);
		return 1;
	}
	...
	dev_notice(dev, "%s: img revctrl:0x%04X new revctrl:0x%04X\n",
...
//#ifdef KCTP_CUST
	ktp_fw_ver_image = fw_ver_new;
	ktp_fw_revctrl_image = fw_revctrl_new;
//#endif /* KCTP_CUST */
...
//#ifdef KCTP_CUST
	ktp_fw_config_ver_image = cfg_ver_new;
//#endif /* KCTP_CUST */
	dev_notice(dev, "%s: img_ver:0x%04X new_ver:0x%04X\n",
		__func__, ld->si->cydata.fw_ver_conf, cfg_ver_new);
...
#else
	/* Check if config version is match */
	if (cfg_ver_new != ld->si->cydata.fw_ver_conf) {
		dev_notice(dev, "%s: Config version is missmatch, will upgrade\n",
		__func__);
		rc = cyttsp5_upgrade_ttconfig(dev, start, length);
		ld->builtin_bin_cfg_status = 0;
	}
