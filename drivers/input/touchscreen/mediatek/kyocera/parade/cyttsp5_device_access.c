		...
		struct device_attribute *attr, char *buf)
{
	...
	elem = scd->electrodes_x * scd->electrodes_y;

//	pr_info("%s: Rx=%d Tx=%d\n", scd->electrodes_x, scd->electrodes_y);
...
put_pm_runtime:
	...
	size = length;
	index += scnprintf(buf, CY_MAX_PRBUF_SIZE, "%02X", status);
	pr_err("%s:[KCTEST]index=%d status=%d", __func__, index, status);
	...
	index += scnprintf(&buf[index], CY_MAX_PRBUF_SIZE - index,
			"\n");
	pr_err("%s:[KCTEST]index=%d length=%d", __func__, index, length);
	size = index;
...
static DEVICE_ATTR(get_mu_se_diff, S_IRUSR|S_IRGRP, cyttsp5_mutual_self_diff_show, NULL);
...
put_pm_runtime:
	...
	index += scnprintf(buf, CY_MAX_PRBUF_SIZE, "%02X", cmd_status);
	pr_err("%s:[KCTEST]index=%d status=%d", __func__, index, cmd_status);
